...
import { apiGetBoxMaterial, apiUpdateBoxMaterial, type ApiStatus } from '@/lib/api';
...
const EditMaterialPage: React.FC = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setLoading(true);
        try {
            await apiUpdateBoxMaterial(id!, values);
            navigate('/materials');
        } catch (err: any) {
        ...
        <ProtectedRoute requiredPermissions={['boxes:edit']}>
            <Box>
                <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
                    ...
                    <Typography variant="h5">Edit Material</Typography>
                </Box>
                    ...
                    {values && (
                        <>
                            ...
                            <Box sx={{ mt: 2, display: 'flex', justifyContent: 'flex-end', gap: 1.5 }}>
                                <Button type="button" variant="outlined" color="inherit" onClick={() => navigate('/materials')}>Cancel</Button>
